...
function chooseFamily(r,family,difficulty){let list=TYPES.filter(t=>t.minStage<=r.stage&&t.difficulty<=difficulty+1&&t.family===family);if(!list.length){if(family==='wide')list=TYPES.filter(t=>t.id==='log-pile');else if(family==='high')list=TYPES.filter(t=>t.id==='stump');else if(family==='overhead')list=TYPES.filter(t=>t.id==='branch');else if(family==='flex')list=TYPES.filter(t=>t.id==='canopy');else list=TYPES.filter(t=>t.id==='log')}return list[Math.floor(randomValue(r)*list.length)]||TYPES[0]}
...
export function step(r,dt){if(r.dead)return[];const events=[];r.time+=dt;const next=stageAt(r.time);if(next!==r.stage){r.stage=next;r.stageChanged=1.8;r.targetSpeed=targetSpeed(r.stage);r.notice=`STAGE ${String(r.stage).padStart(2,'0')} · ${BIOMES[(r.stage-1)%BIOMES.length][0]}`;r.noticeTime=1.8;r.spawnDistance=Math.max(r.spawnDistance,r.speed*1.2);events.push('stage')}r.stageChanged=Math.max(0,r.stageChanged-dt);r.targetSpeed=targetSpeed(r.stage);r.speed=r.time<30?BASE_SPEED:r.speed+(r.targetSpeed-r.speed)*Math.min(1,dt/2.5);const wasDashing=!!r.dashing;r.modeTime=Math.max(0,r.modeTime-dt);r.dashing=r.modeTime>0;r.dashTrail=r.modeTime>0?Math.min(1,(r.dashTrail||0)+dt*7):Math.max(0,(r.dashTrail||0)-dt*3);if(!wasDashing&&r.dashing)events.push('dash-start');else if(wasDashing&&!r.dashing)events.push('dash-end');r.flow=Math.max(0,r.flow-dt*(r.modeTime>0?1:2.5));r.invulnerable=Math.max(0,r.invulnerable-dt);r.shieldHit=Math.max(0,r.shieldHit-dt);r.shake=Math.max(0,r.shake-dt);r.slowMo=Math.max(0,r.slowMo-dt);if(r.time>=r.nextEvent&&!r.event&&r.stage>=3)startEvent(r);if(r.event){r.event.time-=dt;if(r.event.time<=0)r.event=null}const dx=r.speed*dt;r.distance+=dx/100;r.noticeTime=Math.max(0,r.noticeTime-dt);if(movePlayer(r,dt))events.push('land');r.spawnDistance-=dx;if(r.spawnDistance<=0)spawnPattern(r);for(const o of r.obstacles){o.x-=dx;updateObstacleMotion(o,r.time);if(collides(r,o)){if(r.shield){r.shield=false;r.shieldBreaks++;r.invulnerable=.7;r.shieldHit=.34;r.shake=.22;r.slowMo=.12;r.flow=Math.max(0,r.flow-35);r.combo=0;o.resolved='hit';o.x=-o.w-50;events.push('shield-used')}else{r.dead=true;r.playerState='game-over';events.push('dead');break}}checkResolution(r,o,events)}r.obstacles=r.obstacles.filter(o=>o.x+o.w+(o._mx||0)>-40);for(const b of r.bananas){b.x-=dx;if(!b.collected&&Math.abs(b.x-PLAYER_X)<27&&Math.abs(b.y-(r.y+(playerBox(r).sliding?18:48)))<31){b.collected=true;r.bananaCount++;const value=b.golden?500:25;r.goldenBananas+=b.golden?1:0;r.bonus+=Math.round(value*flowMultiplier(r.flow));r.flow=Math.min(100,r.flow+(b.golden?30:3));r.maxFlow=Math.max(r.maxFlow,r.flow);events.push(b.golden?'golden':'banana')}}r.bananas=r.bananas.filter(b=>!b.collected&&b.x>-35);if(r.time>=r.nextPower&&!r.power){r.power={x:1080+reactionDistance(r.speed,.8),y:126};r.nextPower=r.time+23+randomValue(r)*7}if(r.power){r.power.x-=dx;if(Math.abs(r.power.x-PLAYER_X)<34&&Math.abs(r.y+42-r.power.y)<43){r.shield=true;r.power=null;events.push('power')}else if(r.power.x<-40)r.power=null}if(r.time>=r.nextSplit){r.splits.push({time:Math.floor(r.time),distance:r.distance});r.nextSplit++}r.score=Math.floor(r.distance*10)+r.bonus;return events}
