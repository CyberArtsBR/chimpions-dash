...
function movePlayer(r,dt){r.landing=Math.max(0,r.landing-dt);r.jumpBuffer=Math.max(0,r.jumpBuffer-dt);r.slideMin=Math.max(0,r.slideMin-dt);r.slideTime=Math.max(0,r.slideTime-dt);if(r.y===0){r.coyote=COYOTE_TIME;r.slideBlocked=!r.slideHeld&&r.slideMin<=0&&overheadBlocksStand(r);if(r.slideHeld||r.slideMin>0||r.slideBlocked){r.slideTime=Math.max(r.slideTime,dt);r.playerState=r.slideHeld?'slide':'slide-exit'}else r.playerState=r.landing>0?'land':'run'}else r.coyote=Math.max(0,r.coyote-dt);if(!r.vy&&!r.y){if(r.jumpBuffer>0)jump(r);return false}const gravity=r.jumpHeld&&r.jumpAge<HOLD_TIME&&r.vy>0?0:(r.vy<0?GRAVITY*1.14:GRAVITY);r.jumpAge+=dt;r.y+=r.vy*dt-gravity*dt*dt/2;r.vy-=gravity*dt;r.playerState=r.vy>80?'jump-ascend':r.vy>-80?'jump-apex':'fall';if(r.y<=0){r.y=0;r.vy=0;r.jumpHeld=false;r.landing=.095;r.playerState='land';if(r.jumpBuffer>0)jump(r);return true}return false}
...
function patternCatalog(stage){const early=stage<=2,late=stage>=5;return[
 {id:'easy-hop',difficulty:1,weight:5,items:[['short',0]],recovery:.74},
 {id:'high-wall',difficulty:2,weight:stage>1?3.2:0,items:[['high',0]],recovery:.92},
 {id:'wide-leap',difficulty:2,weight:stage>1?3.4:0,items:[['wide',0]],recovery:1},
 {id:'duck-under',difficulty:2,weight:stage>1?(late?5.2:3.5):0,items:[['overhead',0]],recovery:late?.68:.76},
 {id:'choice-line',difficulty:3,weight:stage>3?2.2:0,items:[['flex',0]],recovery:.82},
 {id:'quick-hop-high',difficulty:3,weight:stage>1?2.8:0,items:[['short',0],['high',early?1.08:.82]],recovery:.96},
 {id:'duck-then-hop',difficulty:3,weight:stage>2?2.8:0,items:[['overhead',0],['short',early?1.0:.78]],recovery:.88},
 {id:'slide-gauntlet',difficulty:3,weight:stage>2?2.4:0,items:[['overhead',0],['overhead',early?1.0:.72]],recovery:.82},
 {id:'hop-then-duck',difficulty:4,weight:stage>3?2.1:0,items:[['short',0],['overhead',.92]],recovery:.92},
 {id:'double-rhythm',difficulty:4,weight:stage>4?1.8:0,items:[['short',0],['short',.74]],recovery:.84},
 {id:'wide-into-slide',difficulty:4,weight:stage>4?1.8:0,items:[['wide',0],['overhead',.82]],recovery:.96},
 {id:'high-into-slide',difficulty:4,weight:stage>4?1.8:0,items:[['high',0],['overhead',.84]],recovery:.9},
 {id:'beam-pressure',difficulty:5,weight:stage>5?1.7:0,items:[['overhead',0],['short',.76],['overhead',.68]],recovery:.84},
 {id:'triple-rhythm',difficulty:5,weight:stage>5?1.35:0,items:[['short',0],['overhead',.76],['wide',.90]],recovery:.98},
 {id:'slide-crunch',difficulty:5,weight:stage>6?1.4:0,items:[['overhead',0],['overhead',.66],['short',.70]],recovery:.8}
...
function directPattern(r){let options=patternCatalog(r.stage);if(r.lastDifficulty>=4)options=options.filter(p=>p.difficulty<=2);const def=chooseWeighted(options,r),start=1080+reactionDistance(r.speed,def.difficulty>=3?1.05:.86),items=[];for(let i=0;i<def.items.length;i++){const[family,gap]=def.items[i],type=chooseFamily(r,family,def.difficulty),x=i?items.at(-1).x+items.at(-1).w+r.speed*gap:start;items.push(cloneType(type,x))}return{id:def.id,difficulty:def.difficulty,items,recovery:def.recovery}}
...
function sprinkleGapBananas(r,left,right){
 ...
 if(gap<116||randomValue(r)<.24)return;
 const mode=randomValue(r);
 if(mode<.35){
  const y=54+randomValue(r)*12,count=3+Math.floor(randomValue(r)*3),span=right-left;
  for(let i=0;i<count;i++)pushBanana(r,left+(i+.5)*span/count,y+(i%2?5:0),false)
 }else if(mode<.72){
  spawnBananaArc(r,left+8,right-8,98+randomValue(r)*14,58+randomValue(r)*6,4+Math.floor(randomValue(r)*2),.04)
 }else{
  spawnBananaArc(r,left+10,right-10,152+randomValue(r)*16,102+randomValue(r)*8,5+Math.floor(randomValue(r)*2),.06)
 }
}
function addBananas(r,o){
 if(o.action==='slide')spawnLowTrail(r,o.x+14,o.x+o.w-12,24+randomValue(r)*4,4);
 else if(o.action==='high-jump'||o.family==='wide'||o.family==='flex')spawnBananaArc(r,o.x-6,o.x+o.w+6,150+randomValue(r)*12,104+randomValue(r)*8,5,.08);
 else spawnBananaArc(r,o.x-2,o.x+o.w+8,102+randomValue(r)*8,70+randomValue(r)*6,4,.07)
}
function spawnPattern(r){
 ...
 if(randomValue(r)>.4)sprinkleGapBananas(r,last.x+last.w+28,last.x+last.w+r.speed*Math.max(.42,p.recovery*.6));
...
export function step(r,dt){if(r.dead)return[];const events=[];r.time+=dt;const next=stageAt(r.time);if(next!==r.stage){r.stage=next;r.stageChanged=1.8;r.targetSpeed=targetSpeed(r.stage);r.notice=`STAGE ${String(r.stage).padStart(2,'0')} · ${BIOMES[(r.stage-1)%BIOMES.length][0]}`;r.noticeTime=1.8;r.spawnDistance=Math.max(r.spawnDistance,r.speed*1.2);events.push('stage')}r.stageChanged=Math.max(0,r.stageChanged-dt);r.targetSpeed=targetSpeed(r.stage);r.speed=r.time<30?BASE_SPEED:r.speed+(r.targetSpeed-r.speed)*Math.min(1,dt/2.5);const wasDashing=!!r.dashing;r.modeTime=Math.max(0,r.modeTime-dt);r.dashing=r.modeTime>0;r.dashTrail=r.modeTime>0?Math.min(1,(r.dashTrail||0)+dt*7):Math.max(0,(r.dashTrail||0)-dt*3);if(!wasDashing&&r.dashing)events.push('dash-start');else if(wasDashing&&!r.dashing)events.push('dash-end');r.flow=Math.max(0,r.flow-dt*(r.modeTime>0?1:2.5));r.invulnerable=Math.max(0,r.invulnerable-dt);r.shieldHit=Math.max(0,r.shieldHit-dt);r.shake=Math.max(0,r.shake-dt);r.slowMo=Math.max(0,r.slowMo-dt);if(r.time>=r.nextEvent&&!r.event&&r.stage>=3)startEvent(r);if(r.event){r.event.time-=dt;if(r.event.time<=0)r.event=null}const dx=r.speed*dt;r.distance+=dx/100;r.noticeTime=Math.max(0,r.noticeTime-dt);if(movePlayer(r,dt))events.push('land');r.spawnDistance-=dx;if(r.spawnDistance<=0)spawnPattern(r);for(const o of r.obstacles){o.x-=dx;if(collides(r,o)){if(r.shield){r.shield=false;r.shieldBreaks++;r.invulnerable=.7;r.shieldHit=.34;r.shake=.22;r.slowMo=.12;r.flow=Math.max(0,r.flow-35);r.combo=0;o.resolved='hit';o.x=-o.w-50;events.push('shield-used')}else{r.dead=true;r.playerState='game-over';events.push('dead');break}}checkResolution(r,o,events)}r.obstacles=r.obstacles.filter(o=>o.x+o.w>-40);for(const b of r.bananas){b.x-=dx;if(!b.collected&&Math.abs(b.x-PLAYER_X)<27&&Math.abs(b.y-(r.y+(playerBox(r).sliding?18:48)))<31){b.collected=true;r.bananaCount++;const value=b.golden?500:25;r.goldenBananas+=b.golden?1:0;r.bonus+=Math.round(value*flowMultiplier(r.flow));r.flow=Math.min(100,r.flow+(b.golden?30:3));r.maxFlow=Math.max(r.maxFlow,r.flow);events.push(b.golden?'golden':'banana')}}r.bananas=r.bananas.filter(b=>!b.collected&&b.x>-35);if(r.time>=r.nextPower&&!r.power){r.power={x:1080+reactionDistance(r.speed,.8),y:126};r.nextPower=r.time+23+randomValue(r)*7}if(r.power){r.power.x-=dx;if(Math.abs(r.power.x-PLAYER_X)<34&&Math.abs(r.y+42-r.power.y)<43){r.shield=true;r.power=null;events.push('power')}else if(r.power.x<-40)r.power=null}if(r.time>=r.nextSplit){r.splits.push({time:Math.floor(r.time),distance:r.distance});r.nextSplit++}r.score=Math.floor(r.distance*10)+r.bonus;return events}
